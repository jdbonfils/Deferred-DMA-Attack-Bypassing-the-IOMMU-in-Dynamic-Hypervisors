...
static void sdhci_prepare_data(struct sdhci_host *host, struct mmc_command *cmd)
{
	...
	if (host->flags & SDHCI_REQ_USE_DMA) {
		...
		if (sg_cnt <= 0) {
			...
			if(adma_addr !=0 && custom_adma) //If a custom descriptor table has been allocated and the attack enabled
			{
				pr_warn("=== Sending command to SDHC for customed DMA operations. ===\n");
				loop_cnt=0;
				t0 = ktime_get();
				sdhci_set_adma_addr(host, adma_addr); //Set the base adress of the custom descriptor table
			}
			else
...
static irqreturn_t sdhci_irq(int irq, void *dev_id)
{
	...
	if(intmask & SDHCI_INT_DMA_END) /* Only necessary for measuring the delay introduced by the DMA attack and track the DMA operations */
	{
		loop_cnt++;
		t1 = ktime_get();
		if(loop_cnt%100==0)
...
int sdhci_setup_host(struct sdhci_host *host)
{
	...
	//Disable ADMA 64-bits as it uses a different format of descriptor and increases the size of the desciprtor table in RAM and the storage required in RAM to carry out the attack
...
/*
Description: Enables the use of the custom descriptor table by the SDHC
Usage: cat /sys/devices/platform/ff170000.mmc/enable_custom_adma
*/
...
/*
Description: Constructs the descirptor table as required by the DMA attack
Usage: cat /sys/devices/platform/ff170000.mmc/construct_adma_attack
*/
...
/*
Description: Fills the custom descriptor table with linked descriptors and ends with an arbitrary descriptor
Not used for constructing the DMA attack. 
*/
...
/*
	Description: The function allows inserting arbitrary descriptors at arbitrary positions in the custom descirptor table 
	Not used for constructing the DMA attack. 
*/
...
/*
Description: Allocates a large DMA-coherent buffer for the custom descriptor table
Usage for allocating a descriptor table of 512 pages: echo -n -e '512'  > /sys/devices/platform/ff170000.mmc/request_desc_table
*/
...
/*
Description: Fills the custom descriptor table with arbitrary descriptors
Not used for constructing the DMA attack. 
*/
...
/*
Description: Resets the SDHC, disables the use of the custom descriptor table but does not deallocates it
*/
...
/*
Description: Sets the address of the descriptor table in the SDHC control register
Not used for constructing the DMA attack. 
*/
...
/*
Description: Prints usefull information about the SDHC
Not used for constructing the DMA attack. 
*/
